...
from core.config import ADMIN_LIST_PATH, GROUPS_PATH, PLAYERS_LIST_PATH


admin_list = ADMIN_LIST_PATH
player_data = PLAYERS_LIST_PATH
...
def writer_group_to_json(data, filename=GROUPS_PATH):
    try:
...
def remove_group_from_json(username, filename=GROUPS_PATH):
    try:
        # Читаем json файл
        with open(filename, 'r', encoding='utf-8') as f:
            existing_data = json.load(f)
...
def load_from_json(filename=GROUPS_PATH):
    try:
        with open(filename, 'r', encoding='utf-8') as f:
            return json.load(f)
    except FileNotFoundError:
